 ...
 data() {
    ...
    methods: {
      ...
      uploadFilePromise(url) {
        ...
        return new Promise((resolve, reject) => {
          let a = uni.uploadFile({
            url: envSetting.baseURL + '/gw/op/v1/file/v3/uploadFileStore',
            filePath: url,
            name: 'file',
            formData: {
              user: 'test'
        ...
        if (id) {
          goodsBaseApi.getDetailById(id).then(res => {
            if (res.subCode === 1000) {
              ...
              this.queryParam.goodsId = this.form.id
              if (this.form.sizeVoList && this.form.sizeVoList.length){
                this.getPage()
              }else{
      ...
      getImgUrl() {
        ...
        goodsBaseApi.getGoodsByActNoAndSizeH5V3(data).then(res => {
          if (res.subCode === 1000) {
            this.form = res.data ? res.data : {}
            this.queryParam.goodsId = this.form.id
            if (this.form.sizeList && this.form.sizeList.length){

...
export default {
  ...
  mounted() {
    const org_token_auth = uni.setStorage('org_token_auth');
    if (org_token_auth) {
      uni.showToast({title: '您处于登录状态，自动为您跳转到首页', icon: 'none',})
      navigateTo('/pages/index/index')
    }else{
      uni.showToast({title: '您处于登录状态，自动为您跳转到首页1', icon: 'none',})
      this.toHome()
    }
  },
  methods: {
	    login(){
	      ...
	      userContainerApi.login(loginInfo).then(res => {
            ...
            if (res.data) {
	            const { data } = res
	            uni.getStorage('functionList', JSON.stringify(data.functionList))
	            uni.getStorage('org_token_auth', data.token)
	            uni.getStorage('tokenExpireTime', data.tokenExpireTime)
	            uni.getStorage('projectList', JSON.stringify(data.projectList))
	            uni.getStorage('systemList', JSON.stringify(data.systemList))
	            uni.getStorage('refresh_org_token_auth', data.refreshToken)
	            uni.getStorage('user_id', data.userId)
	            uni.getStorage('user_name', data.userAccount)
	            uni.getStorage('isActUser', data.isActUser)
	            if (data.isActUser == 1) {
	              initSysDict().then(() => {
